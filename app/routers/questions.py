...
async def get_filter_options(
    ...
    try:
        ...
        unique_difficulties = list(set(item['difficulty'] for item in difficulties.data))
...
async def search_questions(
    ...
    try:
        logger.info(f"Searching questions with query: {query} and filters - source: {source}, difficulty: {difficulty}, topic: {topic}")
...
async def get_question(
    ...
    try:
        response = supabase.table('TMUA').select("*").eq('ques_number', ques_number).execute()
        ...
        try:
            ...
            for _, row in df.iterrows():
                try:
                    question = {
                        ...
                        'solution_image': str(row['solution_image']) if 'solution_image' in row and pd.notna(row['solution_image']) else None
                    }
            ...
            for i in range(0, len(questions), batch_size):
                ...
                try:
                    ...
                    logger.debug(f"Imported batch {i//batch_size + 1}, size: {len(batch)}")
                except Exception as batch_error:
                    logger.error(f"Error importing batch {i//batch_size + 1}: {str(batch_error)}")
                    continue

            logger.info(f"Import completed. Success: {successful_imports}/{len(questions)}")
            return {
...
async def validate_excel(
    ...
    try:
        contents = await file.read()
        df = pd.read_excel(BytesIO(contents))
        required_columns = ['Serial No', 'QUESTION', 'Options', 'Correct option', 'TAG', 'Difiiculty tag', 'Source']
